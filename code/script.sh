...
jq -r '[.[] | {path: .message | fromjson | .servlet.path, method: .message | fromjson | .http.method, dura: .message | fromjson | .duration, duration: (.message | fromjson | .duration / 1e9)}]' "$input_json" | jq -r '.[] | [.method, .path, .dura, .duration] | @csv' > temp.csv
...
worksheet.write(0, 0, "Method")
worksheet.write(0, 1, "API")
worksheet.write(0, 2, "Duration (nanoseconds)")
worksheet.write(0, 3, "Duration (seconds)")
